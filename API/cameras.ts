...
export async function addVirtualVideo(camerasList: { [key: string]: any, "videochannelID": string }[], highStreamVideo: string, lowStreamVideo = '') {
...
export async function getCamerasEndpoints() {
    let request = await fetch(`${serverURL}/camera/list`, {
        headers: {
            "Authorization": `Basic ${ROOT_AUTH}`,
        }
    });
...
export async function setPTZPreset(telemetryAccessPoint: string, position: number, name: string) {

    let body = {
        "method": "axxonsoft.bl.ptz.TelemetryService.SetPreset",
        "data": {
            "access_point": telemetryAccessPoint,
            "position": position,
            "label": name
        }
    };
    ...
    if (request.ok) {
        console.log(`Preset #${position} "${name}" was set to endpoint ${telemetryAccessPoint}.`.green);
    } else console.log(`Error: Couldn't set preset for ${telemetryAccessPoint}. Code: ${request.status}`.red);
};

export async function setPanamorphMode(videoChannelEndpoint: string, panamorphState: boolean) {
    ...
    if (request.ok && !response.failed.length) {
        console.log(`Panamorph mode for camera "${videoChannelEndpoint}" was ${panamorphState ? 'enabled' : 'disabled'}.`.green);
    } else console.log(`Error: Coudn't set panamorph mode for camera "${videoChannelEndpoint}". Code: ${request.status}, Failed: ${response.failed}`.red);
};