...
export async function setObjectPermissions(roleName: string, cameraEndpoints: string[], accessLevel: "CAMERA_ACCESS_FORBID" | "CAMERA_ACCESS_ONLY_ARCHIVE" | "CAMERA_ACCESS_MONITORING_ON_PROTECTION" | "CAMERA_ACCESS_MONITORING" | "CAMERA_ACCESS_ARCHIVE" | "CAMERA_ACCESS_MONITORING_ARCHIVE_MANAGE" | "CAMERA_ACCESS_FULL" = "CAMERA_ACCESS_FULL") {
    let camerasObj: object = {};

    for (let cameraEndpoint of cameraEndpoints){
        camerasObj[cameraEndpoint] = accessLevel;
    }
    ...
    if (request.ok) {
        console.log(`Cameras ${cameraEndpoints.toString()} for role "${roleName}" was assigned access ${accessLevel}!`.green);
